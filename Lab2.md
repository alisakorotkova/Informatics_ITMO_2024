...
6. Последние действие  - это вывод конечного результата с помощью команды `echo`. Но так как в строке присутствует лишняя точка вначале строки, то выводим только срез (строку без первого элемента).
